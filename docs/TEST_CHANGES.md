...
- `051_fix_test_positions_gender.sql` `3c837dc` — fixes `TEST_ELECTION_DELETE_ME` `positions` `gender NULL` -> `Male/Female`, reassigns `Ctrlplusz069` `Female` from `Boys` to `Girls`.
- `052_recreate_after_049_fix_gender.sql` `3c837dc` — recreates `TEST_ELECTION_DELETE_ME` + `8 voters` + re-inserts `Who07512 -> Boys`, `Ctrlplusz069 -> Girls` after `049` prematurely deleted it. Run order on Render: `049` then `051` then `052` (049 deleted old, 052 rebuilds correctly; future real cleanup uses the unpushed final `049` variant only after live test passes).
- `053_set_passwords_for_15_live_test_accounts.sql` (this commit) — all `15` whitelisted accounts had `password_hash NULL`, so student + candidate email/password login ALWAYS returned `Invalid username or password.` Sets verified `scrypt` hash for `TestPassword123!` (same as `043`) + `username` (email local-part) + `password_change_required=false` + clears failed attempts/lock. **Login:** `email` + `TestPassword123!` works in both `/student/login` and `/candidate/login` (frontend sends no role; backend routes by account role STUDENT→`/candidate/status`). **Revert after test:** set `password_hash=NULL`, `username=NULL`.
